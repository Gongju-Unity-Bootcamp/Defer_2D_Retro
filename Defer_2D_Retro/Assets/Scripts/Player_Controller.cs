...
public class Player_Controller : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        // 몬스터에게 닿을 경우 피격
        if (collision.collider.CompareTag("Summons") && !collision.collider.GetComponent<Monster_Health>().isDead && !PH.isDead)
        {
            // 부딪친 몬스터의 컴포넌트에서 데미지 값을 가져옴
            // 객체별 데미지를 위한 설정
            PH.TakeDamage(collision.collider.GetComponent<Summons_Controller>().damage);
        ...
        // 보스 몬스터에게 닿을 경우 피격
        if (collision.collider.CompareTag("Boss") && !collision.gameObject.GetComponent<Monster_Health>().isDead && !PH.isDead)
        {
            // 부딪친 몬스터의 컴포넌트에서 데미지 값을 가져옴
            // 객체별 데미지를 위한 설정
